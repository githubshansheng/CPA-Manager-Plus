...
func SaveUpdateCheck(ctx context.Context, db *sql.DB, data []byte) error {
	_, err := outboxcontext.Exec(
		...
		"insert into settings(`key`, `value`, `updated_at_ms`) values(?, ?, ?)"+
			sqldialect.ForBackend(database.BackendSQLite).UpsertClause([]string{"key"}, []string{"value", "updated_at_ms"}),
		updateCheckKey,
		string(data),
		time.Now().UnixMilli(),
	)
...
func ClaimUpdateNotification(ctx context.Context, db *sql.DB, tag string) (bool, error) {
	...
	result, err := outboxcontext.Exec(
		ctx,
		db,
		"insert into settings(`key`, `value`, `updated_at_ms`) values(?, json_object('release_tag',?, 'notified_at',?, 'dismissed_at', null), ?)"+
			sqldialect.ForBackend(database.BackendSQLite).InsertDoNothingClause([]string{"key"}, "value"),
		"manager_update_notification_v1:"+tag,
		tag,
		now,
		now,
	)
...
func DismissUpdateNotification(ctx context.Context, db *sql.DB, tag string) error {
	_, err := outboxcontext.Exec(ctx, db, "update settings set value=json_set(value,'$.dismissed_at',?),updated_at_ms=? where `key`=?", time.Now().UnixMilli(), time.Now().UnixMilli(), "manager_update_notification_v1:"+tag)
	return err
}
